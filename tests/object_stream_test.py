...
def test_stream_objects_from_cmd():
   ...
   res = sorted(res, key= lambda o: o['kind'])
   assert res[0]['kind'] == 'ConfigMap'
   assert res[0]['metadata']['name'] == 'ldap-configmap'
   assert res[1]['kind'] == 'Deployment'
   assert res[1]['metadata']['name'] == 'ldap'
   assert res[2]['kind'] == 'Service'
   assert res[2]['metadata']['name'] == 'ldap-service'
